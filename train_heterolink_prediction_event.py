...
from dataloader_event import get_HeteroData, split_heterodata
from neighbor_finder import get_neighbor_finder, get_hetero_neighbor_finder
from event_constructor_heterograph_event import get_batch_of_data, get_example_data
...
full_data, n_nodes = get_HeteroData('./data/{}/ml_{}_connected_5_v3.csv'.format(dataset, dataset), paper_hypergraph_metadata2, data_num=80000000)
...
tgn = TGN(n_nodes, device, edge_feature, embedding_dimension=32, metadata=example_event_data.metadata(), deliver_to='self')
...
for epoch in range(200):
    ...
    for _ in range(1):
        ...
        for idx, (batch_event_data, batch_labels) in tqdm(enumerate(
            ...
            venue_pos_score, venue_neg_score = train_one_batch_link(batch_event_data.to(device))
    ...
    for idx, (batch_event_data, batch_labels) in tqdm(enumerate(
        ...
        venue_pos_score, venue_neg_score = eval_one_batch_link(batch_event_data.to(device))
