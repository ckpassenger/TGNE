...
from model.event_tgn import TGN , MLP

from dataloader import get_Data, split_data_node
...
get_checkpoint_path = lambda \
        epoch: f'./saved_checkpoints/checkpoint-reddit-{epoch}.pth'

full_data = get_Data('./data/{}/ml_{}.csv'.format(dataset, dataset), paper_hypergraph_metadata2, data_num=1170000000049)

print('full', len(full_data.ranked_event))

train_data, val_data, test_data = split_data_node(full_data, head_rate=0.0, tail_rate=1.0)
...
node_decoder = MLP(args.n_hid,drop=0.3)
node_decoder = node_decoder.to(device)
tgn.load_state_dict(torch.load(get_checkpoint_path(0)), strict=True)
...
node_optimizer = torch.optim.Adam(node_decoder.parameters(), lr=3e-6)
...
def evaluate_node(pred_class, label):
    pred_class = torch.sigmoid(pred_class)
    auc_score = roc_auc_score(label.detach().cpu().numpy().flatten().astype('int'),
                              pred_class.cpu().numpy().flatten())
...
def train_one_batch_node(event_graph, label):
    node_x = event_graph.collect('x')  # for feature & memory
    node_mask = event_graph.collect('node_mask')  # 0 for current event center node, 1 for event nodes, 2 for neighbors
    node_negative = event_graph.collect('negative')  # 0 for current event node, 1 for negative sample
    node_timestamp = event_graph.collect('timestamp')
    ...
    optimizer.zero_grad()
    loss = 0
    pos_pool = tgn.predict(node_x, node_mask, node_negative, node_timestamp, edge_index,
                                                           edge_rel_times, edge_idx, edge_mask, batch)
    # print(out[:2], label[:2])
    pred_class = node_decoder(pos_pool.detach())

    # print(torch.mean(pos_score), torch.mean(neg_score))
    loss += 1 * classification_criterion(pred_class, label.flatten())

    loss.backward()
    # torch.nn.utils.clip_grad_norm_(tgn.parameters(), 0.5, norm_type=2)
    node_optimizer.step()
    # print(torch.mean(pos_score.detach()), torch.mean(neg_score.detach()))
    return pred_class.detach()


def eval_one_batch_node(event_graph, label):
    node_x = event_graph.collect('x')  # for feature & memory
    node_mask = event_graph.collect('node_mask')  # 0 for current event center node, 1 for event nodes, 2 for neighbors
    node_negative = event_graph.collect('negative')  # 0 for current event node, 1 for negative sample
    node_timestamp = event_graph.collect('timestamp')
    ...
    pos_pool = tgn.predict(node_x, node_mask, node_negative, node_timestamp, edge_index,
                           edge_rel_times, edge_idx, edge_mask, batch)
    # print(out[:2], label[:2])
    pred_class = node_decoder(pos_pool)
...
for epoch in range(200):
    ...
    for _ in range(10):
        ...
        for idx, (batch_event_data, batch_labels) in tqdm(enumerate(
                get_batch_of_data(train_data, paper_hypergraph_metadata2, ngf, val_rand_sampler, event_per_batch=batch_size)),
                total=batch_num):
            pred_class = train_one_batch_node(batch_event_data.to(device), batch_labels.to(device))

            pos_scores.append(pred_class)
            labels.append(batch_labels)

        pos_scores = torch.cat(pos_scores, dim=0)
        labels = torch.cat(labels, dim=0)

        auc_score = evaluate_node(pos_scores, labels)
        print('train epoch', epoch, 'auc', auc_score)
        logging.info('train epoch'+ str(epoch)+ 'auc'+ str(auc_score))
        '''
        val
        '''
        tgn.eval()
        ...
        for idx, (batch_event_data, batch_labels) in tqdm(enumerate(
                ...
                total=batch_num):
            pred_class = eval_one_batch_node(batch_event_data.to(device), batch_labels)
        ...
        auc_score = evaluate_node(pos_scores, labels)
        if best_val_auc_score<auc_score:
            best_val_auc_score = auc_score
            print('######################## best #################################')
            print('######################## best #################################')
            print('######################## best #################################')
            logging.info('######################## best #################################')
        ...
        print('val epoch',epoch, 'auc', auc_score)
        logging.info('val epoch'+ str(epoch)+ 'auc'+ str(auc_score))
        ...
        labels = torch.cat(labels, dim=0)
        auc_score = evaluate_node(pos_scores, labels)
        print('test epoch', epoch, 'auc', auc_score)
        logging.info('test epoch'+ str(epoch)+ 'auc'+ str(auc_score))
